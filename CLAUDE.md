...
- `vacation.py` — schedules a flat (rate 0) break across every configured goal by rewriting `roadall`. Dispatch-only, dry-run by default.
...
`BEEMINDER_GOALS` (autoratchet) is a comma list with optional per-goal buffer overrides: `"duolingo:1,lifts:3,running2:4,habits"`. Entries without `:N` fall back to `MAX_BUFFER_DAYS`. `safebuf` is days you can do *nothing* without derailing, so `:1` grants one free day and `:0` makes every day an eep day — use 0 for daily habits (`duolingo`, `headspace`, `habits`) and higher buffers for lower-frequency goals (e.g. 4 for a 2x/week goal) to preserve scheduling flexibility. Ratcheting to 0 requires `beemergency: true`, and is irreversible inside the akrasia horizon. Live values live in `goals.conf`, which is the source of truth for which goals are managed — both `autoratchet.yml` and `vacation.yml` load it into `BEEMINDER_GOALS`, so a goal cannot be ratcheted but missed by a vacation break. `vacation.py` ignores the `:N` suffixes.

Beeminder has no archive, pause, or break endpoint; the only lever is `PUT /users/:u/goals/:g.json` with `roadall`, which "must not make the goal easier between now and the akrasia horizon" (today + 7 days). So a break can only be scheduled a week out, never started today — stopping a goal now means archiving it in the UI. `vacation.py` emits history in value form and everything from the break onwards in rate form, which resumes the original slopes and avoids interpolating road values.
